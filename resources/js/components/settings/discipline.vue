  ...
  <!--Insert Modal-->
    ...
    <b-modal ref="category-insert-modal" hide-footer title="Tetapan">
            ...
            <form class="user" @submit.prevent="categoryInsert">
              <b-row>
                <b-col>
                  <label for=""><b>Disiplin</b></label>
              <input type="text" class="form-control" id="exampleInputFirstName" placeholder="Sila masukkan disiplin" v-model="form.discipline">
              <small class="text-danger" v-if="errors.discipline"> {{ errors.discipline[0] }} </small>   
                </b-col>
            </b-row>
  ...
  <b-modal ref="update-modal" hide-footer title="Kemaskini Kod Peruntukan">     
           <form class="user" @submit.prevent="categoryUpdate">
               <b-row>
                <b-col>
                  <label for=""><b>Disiplin</b></label>
              <input type="text" class="form-control" id="exampleInputFirstName" placeholder="Sila masukkan disiplin" v-model="forms.discipline">
              <small class="text-danger" v-if="errors.discipline"> {{ errors.discipline[0] }} </small>   
                </b-col>
            </b-row>
            <br>        
...
<b-row>
   <div class="col">
    ...
    <b-col class="shadow-sm  bg-white">
           <caption><h5><b>Disiplin</b></h5></caption>  
...
//category
        ...
        categoryitems: [],
        categoryfields: [
          { key: 'discipline', label: 'Disiplin', sortable: true, sortDirection: 'asc' },
          { key: 'actions', label: '' },
...
//delete
       ...
       deleteCategory(id){
              ...
              if (result.value) {
                axios.delete('/api/discipline/'+id+ '?token='+ localStorage.getItem('token'))
               .then(() => {
                this.allCategory();
...
// show insert modal
 ...
 // show update modal
       toggleModal(id) {
         axios.get('/api/discipline/'+id+ '?token='+ localStorage.getItem('token'))
 ...
 //insert  
       categoryInsert(){
       axios.post('/api/discipline'+ '?token='+ localStorage.getItem('token'),this.form)
       .then(() => { 
 ...
 //update       
     categoryUpdate(){
       ...
       axios.patch('/api/discipline/'+id+ '?token='+ localStorage.getItem('token'),this.forms)
       .then(() => { 
