  ...
  <!--Insert Modal-->
    ...
    <b-modal ref="category-insert-modal" hide-footer title="Tetapan">
            ...
            <form class="user" @submit.prevent="categoryInsert">
              <b-row>
                <b-col>
                  <label for=""><b>Ward</b></label>
              <input type="text" class="form-control" id="exampleInputFirstName" placeholder="Sila masukkan wad" v-model="form.ward">
              <small class="text-danger" v-if="errors.ward"> {{ errors.ward[0] }} </small>   
                </b-col>
            </b-row>
  ...
  <b-modal ref="update-modal" hide-footer title="Kemaskini Kod Peruntukan">     
           <form class="user" @submit.prevent="categoryUpdate">
               <b-row>
                <b-col>
                  <label for=""><b>Ward</b></label>
              <input type="text" class="form-control" id="exampleInputFirstName" placeholder="Sila masukkan wad" v-model="forms.ward">
              <small class="text-danger" v-if="errors.ward"> {{ errors.ward[0] }} </small>   
                </b-col>
            </b-row>
            <br>
...
//delete
       ...
       deleteCategory(id){
              ...
              if (result.value) {
                axios.delete('/api/ward/'+id+ '?token='+ localStorage.getItem('token'))
               .then(() => {
                this.allCategory();
...
// show insert modal
 ...
 // show update modal
       toggleModal(id) {
         axios.get('/api/ward/'+id+ '?token='+ localStorage.getItem('token'))
 ...
 //insert  
       categoryInsert(){
       axios.post('/api/ward'+ '?token='+ localStorage.getItem('token'),this.form)
       .then(() => { 
 ...
 //update       
     categoryUpdate(){
       ...
       axios.patch('/api/ward/'+id+ '?token='+ localStorage.getItem('token'),this.forms)
       .then(() => { 
