  ...
  <!--Insert Modal-->
    ...
    <b-modal ref="category-insert-modal" hide-footer title="Tetapan">
            ...
            <form class="user" @submit.prevent="categoryInsert">
              ...
              <input type="text" class="form-control" id="exampleInputFirstName" placeholder="Sila masukkan mukim" v-model="form.vaccine">
...
<b-row>
   <div class="col">
    ...
    <b-col class="shadow-sm  bg-white">
           <caption><h5><b>Vaksin</b></h5></caption>  
...
//category
        ...
        categoryitems: [],
        categoryfields: [
          { key: 'vaccine', label: 'Vaksin', sortable: true, sortDirection: 'asc' },
...
//delete
       ...
       deleteCategory(id){
              ...
              if (result.value) {
                axios.delete('/api/vaccine/'+id+ '?token='+ localStorage.getItem('token'))
               .then(() => {
                this.allCategory();
...
// show insert modal
 ...
 // show update modal
       toggleModal(id) {
         axios.get('/api/vaccine/'+id+ '?token='+ localStorage.getItem('token'))
 ...
 //insert  
       categoryInsert(){
       axios.post('/api/vaccine'+ '?token='+ localStorage.getItem('token'),this.form)
       .then(() => { 
 ...
 //update       
     categoryUpdate(){
       ...
       axios.patch('/api/vaccine/'+id+ '?token='+ localStorage.getItem('token'),this.forms)
       .then(() => { 
